...
// Define UPLOAD_DIR here so bulk ZIP download can locate CV files.
...
// ── Authentication ────────────────────────────────────────────────────────────
// Check BEFORE requireHrLogin() so the redirect always goes to the correct
// hr_login.php in this folder, regardless of what db_config.php's
// requireHrLogin() does internally (it may redirect to a different path).
startSecureSession();
if (empty($_SESSION['hr_logged_in'])) {
    header('Location: hr_login.php');
    exit;
}
requireHrLogin(); // additional checks from db_config.php (session timeout, etc.)
...
if ($filterQual !== '') {
    // Use distinct placeholders (some PDO drivers disallow reusing the same named placeholder multiple times)
    $where[]           = '(qualification_1 LIKE :qual1 OR qualification_2 LIKE :qual2 OR qualification_3 LIKE :qual3 OR qualification_4 LIKE :qual4)';
    $qualLike          = '%' . $filterQual . '%';
    $params[':qual1']  = $qualLike;
    $params[':qual2']  = $qualLike;
    $params[':qual3']  = $qualLike;
    $params[':qual4']  = $qualLike;
}
if ($filterDateFrom !== '') {
...
if ($filterSearch !== '') {
    // Use distinct placeholders (some PDO drivers disallow reusing the same named placeholder multiple times)
    $where[]        = '(first_name LIKE :s1 OR last_name LIKE :s2 OR email_id LIKE :s3 OR contact_number LIKE :s4)';
...
if (isset($_GET['bulk_zip']) && $_GET['bulk_zip'] === '1') {
    ...
    foreach ($cvRows as $r) {
        $fullPath = UPLOAD_DIR . basename($r['cv_file_path']);
...
// Re-clamp $page now that we know $totalPages, then recalculate $offset.
...
/* Main */
/* NO overflow-x here — any overflow value forces overflow-y:auto too,
   which traps all rows in a 100vh scroll box making old records invisible.
   Horizontal scroll is on the inner table wrapper div only. */
...
<div class="layout">
  ...
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <a href="?bulk_zip=1&<?= buildQS() ?>"   class="nav-item is-hidden" aria-hidden="true" tabindex="-1"><i class="fas fa-file-archive"></i> Bulk Download CVs</a>
    </nav>
  ...
  <main class="main">
    ...
    <div class="topbar">
      ...
      <div class="topbar-actions">
        ...
        <a href="?bulk_zip=1&<?= buildQS() ?>"   class="btn btn-ghost is-hidden" aria-hidden="true" tabindex="-1"><i class="fas fa-file-archive"></i> Bulk CVs ZIP</a>
    ...
    <div class="filter-panel">
      ...
      <form method="GET" action="hr_dashboard.php" id="filter-form">
        <div class="filter-grid">
          <div class="fg">
            <label>Search (Name / Email / Phone)</label>
            <input type="text" name="search" id="filter-search" value="<?= e($filterSearch) ?>" placeholder="Type to search…">
          </div>
          <div class="fg">
          ...
          <div class="fg">
            ...
            <input type="number" name="exp_min" id="filter-exp-min" min="0" value="<?= $filterExpMin ?? '' ?>" placeholder="e.g. 2">
          </div>
          <div class="fg">
            <label>Max Experience (Yrs)</label>
            <input type="number" name="exp_max" id="filter-exp-max" min="0" value="<?= $filterExpMax ?? '' ?>" placeholder="e.g. 10">
          </div>
          <div class="fg">
            <label>Qualification keyword</label>
            <input type="text" name="qual" id="filter-qual" value="<?= e($filterQual) ?>" placeholder="e.g. MBBS, B.Sc">
          </div>
          <div class="fg">
            <label>Applied From</label>
            <input type="date" name="date_from" id="filter-date-from" value="<?= e($filterDateFrom) ?>">
          </div>
          <div class="fg">
            <label>Applied To</label>
            <input type="date" name="date_to" id="filter-date-to" value="<?= e($filterDateTo) ?>">
          </div>
        </div>
        <div class="filter-actions">
          <button type="submit" class="btn btn-primary" id="filter-apply"><i class="fas fa-search"></i> Apply Filters</button>
          <a href="hr_dashboard.php" class="btn btn-ghost" id="filter-clear"><i class="fas fa-times"></i> Clear All</a>
        </div>
      </form>
    </div>
        ...
        <table>
          ...
          <?php foreach ($applications as $app): ?>
            <tr>
              ...
              <td style="font-size:.8rem;color:var(--muted)"><?= (int)$app['id'] ?></td>
              <td style="white-space:nowrap;font-size:.8rem">
    <?php
    $dt = new DateTime($app['applied_on'], new DateTimeZone('UTC'));
    ...
    <span style="color:var(--muted)">
        <?= e($dt->format('h:i A')) ?>
              ...
              <td>
                ...
                <strong><?= e(trim($app['title'] . ' ' . $app['first_name'] . ' ' . ($app['middle_name'] ?? '') . ' ' . $app['last_name'])) ?></strong>
...
(function () {
  ...
  const qualEl   = document.getElementById('filter-qual');

  // Client-side filtering (no reload) for the two "typing" fields that users expect to work instantly.
  // Server-side filtering via "Apply Filters" button remains unchanged for pagination + full dataset filtering.
  if (!form || !searchEl || !qualEl) return;
  ...
  function applyClientFilters() {
    const s = norm(searchEl.value);
    const q = norm(qualEl.value);

    let visible = 0;
    rows.forEach(tr => {
      const text = norm(tr.innerText || tr.textContent || '');
      const okSearch = !s || text.includes(s);
      const okQual   = !q || text.includes(q);
      const show = okSearch && okQual;
  ...
  function debounceApply(ms) {
    ...
    t = window.setTimeout(applyClientFilters, ms);
  }

  searchEl.addEventListener('input', () => debounceApply(200));
  qualEl.addEventListener('input', () => debounceApply(200));

  // Run once on load (handles back/forward cache).
  applyClientFilters();
})();
